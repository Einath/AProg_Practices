#include <stdio.h>
   ...
   void Bancos (int k, int disponible, 
...
int main () {
   ...
   Bancos(0,Cap,&sol,&solOptima,M,N,rend);
   ...
   void Bancos (int k, int disponible, 
      ...
      if (k == N) {
         if (sol->beneficio > solOptima->beneficio) *solOptima = *sol;
      }
      else {
         for (c = 0; c <= minimo(M,disponible); c++) {
            ...
            sol->beneficio += rend[c][k];
      	   Bancos (k+1,disponible-c,sol,solOptima,M,N,rend);
