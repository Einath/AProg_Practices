...
void back(int distancia[PLACES+1][PLACES+1], int RnU[PLACES][COL], int visited[PLACES+2], int variante, solution_step sol[PLACES+2]);
...
void calcula(int RnU[PLACES][COL], int distancia[PLACES+1][PLACES+1], solution_step sol[], int origen, int destino, int index);
...
void main(){
  ...
  if(estrategia == 1)
    ...
    back(distancia, RnU, visited, variante, sol); 
...
void voraz(int distancia[PLACES+1][PLACES+1], int RnU[PLACES][COL], int visited[PLACES+2], int variante, solution_step sol[PLACES+2]){ 
  ...
  /*
   * calcula(RnU, distancia, sol, CAMP, ASTAPOR, 1); 
   * calcula(RnU, distancia, sol, ASTAPOR, BHORASH, 2); 
   * calcula(RnU, distancia, sol, BHORASH, YUNKAI, 3); 
   * calcula(RnU, distancia, sol, YUNKAI, MEEREEN, 4); 
   * calcula(RnU, distancia, sol, MEEREEN, CAMP, 5); 
   */
  ...
  if(variante == 1){
    ...
    for(i=1; i<=PLACES; i++){
      ...
      calcula(RnU, distancia, sol, current, destination, i); 
      visited[destination]=TRUE; 
    }
    current = sol[PLACES].position; 
    calcula(RnU, distancia, sol, current, CAMP, PLACES+1); 
  }
