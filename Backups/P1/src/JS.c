...
int years(int X[], int Y[], int lix, int lsx, int liy, int lsy){
  ...
  /* 5.1 |X| ==2 , |Y| >2 */
    ...
    if((Y[posY] >= X[lix]) && (Y[posY] <= X[lsx]))
      ...
      /*Comprobamos paridad*/
      if((lsy-liy)%2 == 0) /* Es de longitud impar */
	return max(Y[posY-1], X[lsx]);
      else /*Es de longitud par */
	return max(Y[posY-2], X[lsx]);
    }
  ...
  /* 5.2 |X| > 2 , |Y| == 2 */
    ...
    if((X[posX] >= Y[liy]) && (X[posX] <= Y[lsy]))
      ...
      /*Comprobamos paridad*/
      if((lsx-lix)%2 == 0) /* Es de longitud impar */
	/* hay que comprobar un caso extra por aquí ------------------ 
ESTÁ JUSTO ANTES O NO INTERVIENE? */
	return max(X[posX-1], Y[lsy]);
      else /*Es de longitud par */
	return max(X[posX-2], Y[lsy]);
    }
